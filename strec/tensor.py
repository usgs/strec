...
def fill_tensor_from_angles(
    strike, dip, rake, magnitude=6.0, source="unknown", mtype="unknown"
):
    ...
    return fill_tensor_from_components(
        mt[0][0],
        mt[1][1],
        mt[2][2],
        mt[1][0],
...
def fill_tensor_from_components(
    mrr, mtt, mpp, mrt, mrp, mtp, source="unknown", mtype="unknown"
):
    ...
    Args:
        ...
        mtype (str): Focal mechanism or moment tensor type (Mww,Mwb,Mwc, etc.)
    Returns:
        tensor_params (dict): Fully descriptive moment tensor dictionary, including
                              fields:
            - mrr,mtt,mpp,mrt,mrp,mtp Moment tensor components.
            - T T-axis values:
              - azimuth (degrees)
...
def plane_to_tensor(strike, dip, rake, mag=6.0):
    ...
    Args:
        ...
        magnitude (float): Magnitude for moment tensor
            (not required if using moment tensor for angular comparisons.)
    Returns:
        mt_matrix (nparray): Tensor representation as 3x3 numpy matrix:
            [[mrr, mrt, mrp]
            [mrt, mtt, mtp]
            [mrp, mtp, mpp]]
    """
